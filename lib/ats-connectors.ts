...
export async function fetchBoardJobs(source: SourceBoard): Promise<CanonicalJob[]> {
  ...
  if (source.ats === "Phenom") {
    // Phenom People career sites (careers.<company>.com) answer their own search widget as public JSON: POST /widgets
    // with ddoKey refineSearch. Slug is "<host>" or "<host>--<lang path>" (job pages live at /<lang path>/job/<id>;
    // default us/en). Country filtering differs per site, so rows are filtered by their location text here.
    const [host, langPath = "us/en"] = source.slug.split("--");
    if (!host) throw new Error("Phenom slug must be host[--lang/path]");
    type PhenomJob = { jobId?: string; reqId?: string; title?: string; cityStateCountry?: string; location?: string; multi_location?: string[]; country?: string; postedDate?: string; dateCreated?: string; applyUrl?: string; descriptionTeaser?: string };
    const seen = new Set<string>();
    const out: CanonicalJob[] = [];
    for (const keywords of ["data engineer", "data platform", "analytics engineer", "machine learning engineer", "data architect"]) {
      for (let page = 0; page < 4; page++) {
        const data = await json<{ refineSearch?: { totalHits?: number; data?: { jobs?: PhenomJob[] } } }>(`https://${host}/widgets`, {
          lang: "en_us", deviceType: "desktop", country: "us", pageName: "search-results", ddoKey: "refineSearch", sortBy: "Most recent", subsearch: "",
          from: page * 50, jobs: true, counts: true, all_fields: ["category", "country", "state", "city", "type"], size: 50, clearAll: false, jdsource: "facets",
          isSliderEnable: false, pageId: "page1", siteType: "external", keywords, global: true, selected_fields: {}, locationData: {},
        });
        const list = data.refineSearch?.data?.jobs ?? [];
        for (const raw of list) {
          const id = str(raw.jobId) || str(raw.reqId);
          if (!id || seen.has(id)) continue;
          seen.add(id);
          const title = str(raw.title);
          const location = str(raw.cityStateCountry) || str(raw.location) || (raw.multi_location ?? []).map(str).join("; ") || str(raw.country);
          if (!keep(title, location)) continue;
          const job = canonical(source, id, title, location, `https://${host}/${langPath}/job/${encodeURIComponent(id)}`, raw.postedDate || raw.dateCreated);
          if (raw.descriptionTeaser) job.jdText = str(raw.descriptionTeaser);
          out.push(job);
        }
