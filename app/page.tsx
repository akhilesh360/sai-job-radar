            ...
            <tbody>
              {filtered.map(job => {
                ...
                return <tr key={job.id}>
                  ...
                  })()}{isNew && <em>NEW</em>}{job.source.includes("(Google)") && <em className="unverified">UNVERIFIED</em>}{job.jdFlags && /no-sponsorship|citizens-only|clearance/.test(job.jdFlags) && <em className="blocker" title={`Description states: ${job.jdFlags.replace(/,/g, ", ")}`}>{job.jdFlags.includes("clearance") ? "CLEARANCE" : job.jdFlags.includes("citizens-only") ? "CITIZENS ONLY" : "NO SPONSORSHIP"}</em>}{job.jdFlags?.includes("sponsorship-offered") && <em className="sponsor-ok" title="Description says visa sponsorship is offered">SPONSORS</em>}</span>{openH1b === job.id && job.h1b && <div className="h1b-detail">
                  ...
                  {(job.jdSkills || job.jdYears) && <span className="jd-tags">{job.jdYears ? <i className="yrs">{job.jdYears}+ yrs</i> : null}{(job.jdSkills ?? "").split(", ").filter(Boolean).slice(0, 7).map(skill => <i key={skill}>{skill}</i>)}</span>}</div></div></td>
