...
type Job = {
  ...
  h1b: { name: string; approvals: number; fiscalYear: number; exact: boolean; lcaLatestFy: number | null; lca?: Array<{ fiscalYear: number; lcas: number; positions: number; dataLcas: number; dataWageP25: number | null; dataWageMedian: number | null; dataWageP75: number | null; topDataTitles: string | null }> } | null;
...
export default function Home() {
  ...
  const [openH1b, setOpenH1b] = useState<string | null>(null);
              ...
              {filtered.map(job => {
                ...
                return <tr key={job.id}>
                  <td><div className="role-cell"><span className="company-avatar">{job.company.slice(0, 2).toUpperCase()}</span><div className="role-body">
                    ...
                    const label = latest ? `H-1B ✓ ${latest.lcas.toLocaleString()} LCAs FY${latest.fiscalYear}${latest.dataLcas ? ` · ${latest.dataLcas} data` : ""}${latest.dataWageP25 && latest.dataWageP75 ? ` · ${k(latest.dataWageP25)}–${k(latest.dataWageP75)}` : ""}` : `H-1B ✓${job.h1b.approvals ? ` ${job.h1b.approvals.toLocaleString()}` : ""}${job.h1b.lcaLatestFy ? ` · FY${job.h1b.lcaLatestFy}` : ""}`;
                    const title = [job.h1b.name, ...(job.h1b.lca ?? []).map(y => `FY${y.fiscalYear}: ${y.lcas.toLocaleString()} certified LCAs (${y.positions.toLocaleString()} positions), ${y.dataLcas} for data/ML roles${y.dataWageMedian ? `, median ${k(y.dataWageMedian)}` : ""}${y.topDataTitles ? ` — ${y.topDataTitles}` : ""}`), job.h1b.approvals ? `USCIS: ${job.h1b.approvals.toLocaleString()} approvals FY${job.h1b.fiscalYear}` : "", prev && latest ? (latest.lcas > prev.lcas ? "Filing more than the year before" : latest.lcas < prev.lcas ? "Filing less than the year before" : "") : "", job.h1b.exact ? "" : "matched by name prefix"].filter(Boolean).join("\n");
                    return <button type="button" className={`h1b${latest && prev && latest.lcas < prev.lcas * 0.6 ? " h1b-down" : ""}${openH1b === job.id ? " open" : ""}`} title={title} aria-expanded={openH1b === job.id} onClick={() => setOpenH1b(open => open === job.id ? null : job.id)}>{label} ▾</button>;
                  })()}{job.jdFlags && /no-sponsorship|citizens-only|clearance/.test(job.jdFlags) && <em className="blocker" title={`Description states: ${job.jdFlags.replace(/,/g, ", ")}`}>{job.jdFlags.includes("clearance") ? "CLEARANCE" : job.jdFlags.includes("citizens-only") ? "CITIZENS ONLY" : "NO SPONSORSHIP"}</em>}{job.jdFlags?.includes("sponsorship-offered") && <em className="sponsor-ok" title="Description says visa sponsorship is offered">SPONSORS</em>}</div>}
                    {openH1b === job.id && job.h1b && <div className="h1b-detail">
                    <div className="h1b-detail-head"><strong>{job.h1b.name}</strong>{!job.h1b.exact && <small>matched by name prefix</small>}</div>
                    {job.h1b.lca && job.h1b.lca.length > 0
                      ? <table><thead><tr><th>Fiscal year</th><th>Certified LCAs</th><th>Positions</th><th>Data / ML roles</th><th>Data wage (median)</th><th>Top data titles</th></tr></thead>
                          <tbody>{job.h1b.lca.map(y => <tr key={y.fiscalYear}><td>FY{y.fiscalYear}</td><td>{y.lcas.toLocaleString()}</td><td>{y.positions.toLocaleString()}</td><td>{y.dataLcas}</td><td>{y.dataWageMedian ? `$${Math.round(y.dataWageMedian / 1000)}k${y.dataWageP25 && y.dataWageP75 ? ` (${Math.round(y.dataWageP25 / 1000)}–${Math.round(y.dataWageP75 / 1000)}k)` : ""}` : "—"}</td><td>{y.topDataTitles ?? "—"}</td></tr>)}</tbody></table>
                      : <p>No certified H-1B Labor Condition Applications found for FY2021–FY2025 under this name.</p>}
                    <p className="h1b-detail-foot">{job.h1b.approvals ? `USCIS Employer Data Hub: ${job.h1b.approvals.toLocaleString()} H-1B approvals in FY${job.h1b.fiscalYear}. ` : ""}Sources: DOL LCA disclosure data (certified cases, FY2021–FY2025) and USCIS H-1B Employer Data Hub. An LCA is the Department of Labor step before an H-1B petition — a strong sign the employer sponsors, not a guarantee for any one role.</p>
                  </div>}
                    {(job.jdSkills || job.jdYears) && <div className="jd-tags">{job.jdYears ? <i className="yrs">{job.jdYears}+ yrs</i> : null}{(job.jdSkills ?? "").split(", ").filter(Boolean).slice(0, 7).map(skill => <i key={skill}>{skill}</i>)}</div>}
                  </div></div></td>
