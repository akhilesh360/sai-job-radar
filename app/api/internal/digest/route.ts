...
export async function POST(){
  ...
  const recent=(await db.select().from(jobs).orderBy(desc(jobs.discoveredAt)).limit(100)).filter(job=>!sentIds.has(job.id)&&!job.isSeed&&isUsLocation(job.location)).slice(0,25);
